  ...
  try {
    ...
    if (retrievedDocs.length > 0) {
      ...
      if (logIds.length > 0) {
        ...
        if (logs?.length) contextText = logs.map(l => l.log_text).join("\n\n---\n\n");
      }
      if (!contextText) contextText = retrievedDocs.map(d => d.pageContent).join("\n\n---\n\n");
    } else {
    ...
    const plannerResult = await runPlanner({ userMessage, context: contextText, userId, googleAuthClient, embeddings, onStatus });
